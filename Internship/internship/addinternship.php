...
<?php
    ...
    if(isset($_POST['btn_add'])){
      ...
      $anh            = $_FILES['c_img']['name']; 
        ...
        if(empty($codeinternship) || empty($nameinternship) || empty($ngaysinh) || empty($sdt) || empty($diachi) || empty($truong) || empty($gioitinh)){
          $loi = "Vui lòng nhập đầy đủ thông tin!";
          $isError = true;
        }else{
            $sqlschool = "SELECT * FROM internship WHERE codestudy = '$codeinternship'";
            $resultschool = $db->query($sqlschool);
            $total = $db->sum($resultschool);
            if($total > 0){
              $loii = "Mã thực tập sinh đã tồn tại!";
              $isError = true;
            }else{
              if(!empty($anh)){
                  $path = "../upload/";
                ...
                $sqlISinternship = "INSERT INTO internship (codestudy, fullname, gender, address, date, schoolid, status, img, phone) 
                VALUES ('$codeinternship', '$nameinternship', '$gioitinh', '$diachi', '$ngaysinh', '$truong', '1', '$anh', '$sdt')";
                $db->add($sqlISinternship);
                echo '<script>alert("Thêm thực tập sinh thành công."); window.location="../internship.php"; </script>';
              }else{
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">ADD Internship</h6>
                        </div>
                            <form action="" method="POST"  enctype="multipart/form-data">
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                        <label for="inputEmail4">Mã thực tập sinh</label>
                                        <input type="text" name="masv" class="form-control" id="inputEmail4" placeholder="Nhập mã thực tập sinh">
                                        </div>
                                        <div class="form-group col-md-6">
                                        <label for="inputPassword4">Họ và tên</label>
                                            <input type="text" name="hovaten" class="form-control" id="inputPassword4" placeholder="Nhập họ và tên">
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <label for="inputPassword4">Ngày sinh</label>
                                            <input type="date" name="ngaysinh" class="form-control" id="inputPassword4" placeholder="Chọn ngày tháng năm sinh">
                                        </div>
                                        <div class="form-group col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="diachi" class="form-control" id="inputAddress" placeholder="Nhập địa địa chỉ">
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <?php
                                                    $sqlgetschool = "SELECT * FROM school WHERE status = 1";
                                                    $resultgetschool = $db->getData($sqlgetschool);
                                                    foreach($resultgetschool as $rowgetschool){
                                                ?>
                                                <option value="<?php echo $rowgetschool['id'];?>"><?php echo $rowgetschool['nameschool'];?></option>
                                                <?php } ?> 
                                                </select>
                                            </div>
                                        </div>
